...
def convert_features():
    ...
    with tqdm(total=data.__len__()) as pbar:
        for data in loader:
            ...
            root = root[0]
            hf_vertices.create_dataset(root, data=vertices, chunks=True, compression="gzip")
            hf_edges.create_dataset(root, data=edges, chunks=True, compression="gzip")
            hf_rank_0.create_dataset(root, data=rank_0, chunks=True, compression="gzip")
            hf_rank_1.create_dataset(root, data=rank_1, chunks=True, compression="gzip")
            hf_rank_2.create_dataset(root, data=rank_2, chunks=True, compression="gzip")
            hf_num_vertices.create_dataset(root, data=num_vertices, chunks=True, compression="gzip")
            hf_num_initial_vertices.create_dataset(root, data=num_initial_vertices, chunks=True, compression="gzip")
            hf_compartment.create_dataset(root, data=compartment, chunks=True, compression="gzip")
            hf_radius.create_dataset(root, data=radius, chunks=True, compression="gzip")
            pbar.update()
