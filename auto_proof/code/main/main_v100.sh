#!/bin/bash
#BATCH --job-name=autoproof_v100    # Job name
#SBATCH --ntasks=1                    # Run on a single process
#SBATCH --gres=gpu:v100:1              # v100 gpu
#SBATCH -c88                  # Need cores
#SBATCH --mem=112gb                     # Job memory request (per node)
...
conda run -n train python -m auto_proof.code.main.main -n 88 -r 112