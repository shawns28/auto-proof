...
#SBATCH --array=1-24
#SBATCH -o auto_proof/data/logs/process_features/%A_%a.out
# %A" is replaced by the job ID and "%a" with the array index
#SBATCH -e auto_proof/data/logs/process_features/%A_%a.err
#SBATCH --partition celltypes         # Partition used for processing

conda run -n auto_env python -m auto_proof.code.pre.process_features -c $SLURM_ARRAY_TASK_ID -n 16